...
export class StrudelMusic {
  ...
  static createMenuTheme(): Pattern {
    try {
      ...
      return stack(melody, harmony, pad, percussion, variations).slow(1);
    } catch (error) {
  ...
  static createEasyMode(): Pattern {
    try {
      ...
      return stack(melody, bass, arpeggios, drums).gain(dynamics).slow(1);
    } catch (error) {
  ...
  static createMediumMode(): Pattern {
    try {
      ...
      const chords = stack(
        note('c3 e3 g3 b3 ~ a2 c3 e3 ~ f3 a3 c4 ~ g2 b2 d3 f3')
          .s('square')
          .lpf(400)
          .gain(0.4)
          .slow(16),
      ...
      return stack(melody, bass, chords, drums, polyrhythm, flourishes).slow(1);
    } catch (error) {
  ...
  static createHardMode(): Pattern {
    try {
      ...
      const melody = note(
        'c4 ~ eb4 f4 ~ g4 bb4 c5 ~ eb5 f5 g5 ~ bb5 c6 ~ ab5 g5 f5 eb5 ~ c5 bb4 g4'
      )
        .s('sawtooth')
      ...
      return stack(melody, bass, chords, drums, arpeggios).gain(dynamics).slow(0.75);
    } catch (error) {
  ...
  static createExpertMode(): Pattern {
    try {
      ...
      const melody = note(
        'c4 d4 eb4 ~ f4 g4 ~ ab4 bb4 ~ c5 d5 ~ eb5 f5 ~ g5 ab5 bb5 ~ c6 bb5 ab5 g5 f5 eb5 d5'
      )
        .s('sawtooth')
      ...
      const harmony = stack(
        note('c3 eb3 gb3 bb3 ~ f3 ab3 cb4 eb4 ~ g3 bb3 db4 f4 ~ c3 eb3 gb3 bb3')
          .s('square')
          .lpf(600)
          .distort(0.3)
          .gain(0.7)
      ...
      return stack(melody, bass, harmony, drums, runs, chaos).slow(0.5);
    } catch (error) {
  ...
  static createZenMode(): Pattern {
    try {
      ...
      const melody = note(
        'c4 ~ e4 ~ g4 ~ b4 ~ d5 ~ g4 ~ e4 ~ c4 ~ ~ ~ a3 ~ c4 ~ e4 ~ g4 ~ b4 ~ d5 ~ ~ ~'
      )
        .s('sine')
        .lpf(600)
        .room(0.8)
      ...
      const pad = stack(
        note('c3 e3 g3 b3 d4 ~ a2 c3 e3 g3 ~ f3 a3 c4 e4 ~ g2 b2 d3 f3 g3')
          .s('sine')
          .lpf(300)
          .room(0.9)
          .gain(0.2)
      ...
      return stack(melody, bass, pad, nature, bells, water).gain(breath).slow(1);
    } catch (error) {
  ...
  static createGameOverMusic(): Pattern {
    try {
      const melody = note('c4 bb3 ab3 g3 f3 ~ ~ ~').s('sine').lpf(500).room(0.6).gain(0.6).slow(8);
  ...
  static createVictoryMusic(): Pattern {
    try {
      const fanfare = note('c4 e4 g4 c5 e5 g5 c6').s('sawtooth').lpf(1200).gain(0.8).slow(2);
  ...
  static createPauseMusic(): Pattern {
    try {
      return note('c4 ~ e4 ~ g4 ~ c5 ~').s('sine').lpf(400).room(0.9).gain(0.3).slow(8);
    } catch (error) {
  ...
  static addVariation(
    basePattern: Pattern,
    variationType: 'subtle' | 'moderate' | 'dramatic' = 'subtle'
  ): Pattern {
    try {
      switch (variationType) {
        case 'subtle':
          // Add subtle high notes occasionally
          const subtleVariation = note('c6 ~ ~ ~ ~ ~ ~ ~').s('sine').gain(0.2).room(0.8).slow(8);
        ...
        case 'dramatic':
          ...
          const dramaticVariation = note('c5 e5 g5').s('sine').gain(0.4).fast(2).slow(4);
