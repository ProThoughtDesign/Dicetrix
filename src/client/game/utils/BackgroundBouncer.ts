...
export class BackgroundBouncer {
  ...
  public getBackgroundPosition(
    ...
    const spriteX = this.logicalPoint.x;
    const spriteY = this.logicalPoint.y;
  ...
  private adjustLogicalPointToBounds(oldBoundingBox: BoundingBox): void {
    ...
    const relativeX =
      (this.logicalPoint.x - oldBoundingBox.left) / (oldBoundingBox.right - oldBoundingBox.left);
    const relativeY =
      (this.logicalPoint.y - oldBoundingBox.top) / (oldBoundingBox.bottom - oldBoundingBox.top);

    // Apply relative position to new bounds
    this.logicalPoint.x =
      this.boundingBox.left + relativeX * (this.boundingBox.right - this.boundingBox.left);
    this.logicalPoint.y =
      this.boundingBox.top + relativeY * (this.boundingBox.bottom - this.boundingBox.top);

    // Ensure position is within bounds (handle edge cases)
    this.logicalPoint.x = Math.max(
      this.boundingBox.left,
      Math.min(this.boundingBox.right, this.logicalPoint.x)
    );
    this.logicalPoint.y = Math.max(
      this.boundingBox.top,
      Math.min(this.boundingBox.bottom, this.logicalPoint.y)
    );
  }
}
